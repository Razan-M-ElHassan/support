	...
	class valid
	{
		...
		*/
		...
		public function Int_min($data,$min)
		{
			...
			if($data < $min)
			{
				return "الرقم $data مفترض ان يكون اقل اكبر او يساوي $min";
		...
		*/
		public function numeric($str)
		{
			if(!is_numeric($str))
			{
				return "النص $str مفترض ان يكون رقم ";
		...
		{
			$urlWithHttp = strpos($data, 'http') !== 0 ? "http://$data" : $data;

			if(filter_var($urlWithHttp,FILTER_VALIDATE_URL) === false)
			{
		...
		public function Date_Min($data,$min,$format = 'YYYY-MM-DD')
		{
			$d=date_create($data);
			if($d === false)
			{
				return "النص $data مفترض ان يكون على الطريقة: $format";
			}
			
			$min = date_create($min);
			
			$diff=date_diff($d,$min);
			$x = $diff->format("%R%a days");
			if($x > 0)
			{
				return "التاريخ ".$data." مفترض ان يكون اكبر من ".$min->format('Y-m-d');
			}
		}
		...
		public function Color($data)
		{
			...
			{
				return "طول النص يفترض ان يكون 4 او 7";
			}
			if($data[0] != "#")
			{
				return "خطأ في صيغة الكود";
			}
		}
